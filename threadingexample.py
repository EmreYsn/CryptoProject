...
async def get_data_async_but_as_wrapper(urls):
    ...
    et = time.time()
    elapsed_time = et - st
    print("Execuation time: ",elapsed_time,"seconds")
    return json_array
...
#get_data_threading(urls) #5
#asyncio.run(get_data_async_but_as_wrapper(urls)) #32
#asyncio.run(get_data_async_concurrently(urls)) #4